...
В данной главе был описан важный компонент системы, который определяет алгоритм ее работы, в этом компоненте скрыта основная возможность системы думать и решать, согласно набором правил и внешним обстоятельствам. \par
...
%===================================
\clearpage
\subsection{Компоненты CoreService.PrelimenaryAnnotator, CoreService.KnowledgeBaseAnnotator, DataService и Reasoner} \label{PreliminaryAnnotator}\label{KnowledgeBaseAnnotator}\label{data service}\label{Reasoner}
...
\textbf{Дерево обучения} базируется на структуре Цель-Подцель. Получение целей происходит из:
\begin{itemize}
	\item Из подцелей;
...
\begin{enumerate}
	...
	\item Компонент Critic Solution Generator (Комопнент генерации решения) запускает KnowingHow WayToThink, ExtensiveSearch (Поиск решения);
